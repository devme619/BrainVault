...
const FilesList = ({ onNoteSelect }) => {
  ...
  const { list: notesList, selectedSubjectTopic } = useSelector((store) => store.notes);

  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const topicId = selectedSubjectTopic ? selectedSubjectTopic.id : null;
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 m-8 border border-dashed border-slate-800 rounded-2xl bg-slate-900/30 text-center backdrop-blur-sm">
        ...
        <h3 className="text-base font-semibold text-slate-300">
          {selectedSubjectTopic ? `No notes in "${selectedSubjectTopic.name}"` : "No notes exist yet"}
        </h3>
        <p className="text-xs text-slate-500 max-w-sm mt-1">
          Click the <strong className="text-emerald-400 font-medium">+ Create Note</strong> button above to add notes to this subject.
        </p>
      </div>
    );
  }
